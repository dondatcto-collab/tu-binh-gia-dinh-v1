...
(function(){
  ...
  const cache=new Map();
  const groupMeaning={AUTHORITY:'trách nhiệm và quy tắc',RESOURCE:'học hỏi và chuẩn bị',OUTPUT:'thực thi và tạo đầu ra',PEER:'phối hợp và tự chủ',WEALTH:'quản lý nguồn lực và tiền bạc'};
  ...
  function whyLine(r){
    ...
    const theme=evidence.find(x=>x?.type==='TEN_GOD_THEME')||{};
    const transit=evidence.find(x=>x?.type==='PERSONAL_TRANSIT')||{};
    const themeName=String(theme.ten_god_vi||theme.theme||'').trim();
    const group=String(theme.theme_group||'').trim();
    const meaning=groupMeaning[group]||'';
    const state=String(transit.state||'').trim();
    if(themeName||group){
      const subject=themeName?`${themeName}${meaning?` — ${meaning}`:''}`:(meaning||group);
      if(state==='SUPPORT')return `Vì: ${subject} đang nổi bật và lớp cá nhân đang hỗ trợ.`;
      if(state==='CAUTION')return `Vì: ${subject} đang nổi bật nhưng lớp cá nhân yêu cầu thận trọng.`;
      return `Căn cứ: ${subject}; trạng thái cá nhân chưa nghiêng rõ.`;
    }
    return 'Căn cứ: chưa có tín hiệu riêng đủ điều kiện cho lĩnh vực này.';
  }
  function summarize(def,r){
    ...
    return {...def,action,caution,confidence,lead,sub,why:whyLine(r),tone:tone(r)};
  }
  function styles(){if(document.getElementById('home-v123-style'))return;const s=document.createElement('style');s.id='home-v123-style';s.textContent=`
    #home-domain-overview-v122,#home-month-summary,#home-today-summary{display:none!important}
    ...
    .home-v123-context{display:flex;gap:8px;padding:10px 14px;border-bottom:1px solid var(--line,#e3ebe8);background:var(--soft,#f6f8f7)}
    .home-v123-context button{border:1px solid var(--line,#d7e2de);background:var(--card,#fff);border-radius:999px;padding:7px 10px;font-size:11px;font-weight:800;color:inherit}
    ...
    .home-v123-main{display:grid;gap:3px;min-width:0}.home-v123-main>b{font-size:14px}.home-v123-lead{font-size:13px;line-height:1.4;font-weight:700}.home-v123-why{font-size:11px;line-height:1.4;color:#315d55}.home-v123-sub{font-size:11px;line-height:1.4;color:var(--muted,#64748b)}
    ...
    @media(max-width:640px){.home-v123-context{overflow:auto}.home-v123-row{grid-template-columns:32px minmax(0,1fr) 16px}.home-v123-state{grid-column:2;justify-self:start}.home-v123-arrow{grid-column:3;grid-row:1 / span 4}.home-v123-icon{grid-row:1 / span 4}}
  `;document.head.appendChild(s)}
  function ensureBox(){const month=$('home-month-summary');if(!month)return null;let box=$('home-action-v123');if(!box){box=document.createElement('section');box.id='home-action-v123';box.className='home-v123';month.parentNode.insertBefore(box,month)}return box}
  function render(rows){const box=ensureBox();if(!box)return;const name=current()?.full_name||'người đang xem';box.innerHTML=`<div class="home-v123-head"><small>HÔM NAY NÊN LÀM GÌ?</small><b>${esc(name)} · hành động có căn cứ</b><p>Chỉ hiện khuyến nghị thật từ từng lĩnh vực; không có căn cứ thì nói thẳng là chưa có.</p></div><div class="home-v123-context"><button onclick="openQuestion('today')">Tổng quan ngày</button><button onclick="openQuestion('month')">Bối cảnh tháng</button></div>${rows.map(x=>`<button class="home-v123-row" onclick="${x.open}"><span class="home-v123-icon">${x.icon}</span><span class="home-v123-main"><b>${esc(x.label)}</b><span class="home-v123-lead">${esc(x.lead)}</span><span class="home-v123-why">${esc(x.why)}</span><span class="home-v123-sub">${esc(x.sub)}</span></span><span class="home-v123-state ${x.tone}">${esc(x.confidence)}</span><span class="home-v123-arrow">›</span></button>`).join('')}<div class="home-v123-cta"><b>Có việc quan trọng?</b> Dùng “Tìm ngày cho một việc” để kiểm theo đúng loại việc thay vì suy từ kết luận chung của hôm nay.</div>`}
  async function load(){if(!currentProfile)return;const box=ensureBox();if(!box)return;box.innerHTML='<div class="home-v123-loading">Đang đọc khuyến nghị cụ thể cho hôm nay…</div>';const p=current();const key=`${p?.profile_id||''}:${todayKey()}`;let data=cache.get(key);if(!data){data=await Promise.all(defs.map(async d=>{try{return summarize(d,await post(d.endpoint,{profile:p,scope:'day'}))}catch(e){return {...d,lead:'Chưa tải được khuyến nghị.',why:'Căn cứ chưa tải được.',sub:e.message||'Hãy thử lại.',confidence:'Lỗi kết nối',tone:'neutral'}}}));cache.set(key,data)}render(data)}
