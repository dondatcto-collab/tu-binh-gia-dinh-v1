...
(function(){
  function plainStatus(raw){
    ...
    if(/Bị chặn|HARD_BLOCK/i.test(x)) return {tone:'bad', short:'Không phù hợp', title:'Không nên chọn thời điểm này cho việc đang xem', meaning:'Có điều kiện chặn ở lớp chọn ngày. Tín hiệu thuận ở lớp cá nhân không được dùng để đảo ngược kết quả này.'};
    ...
    if(/Cần thận trọng|CAUTION/i.test(x)) return {tone:'warn', short:'Thận trọng', title:'Hôm nay nên chậm lại trước các quyết định quan trọng', meaning:'Nền cá nhân trong thời điểm này kém thuận hơn bình thường. Việc thường ngày vẫn có thể làm; việc quan trọng nên kiểm riêng trước khi quyết định.'};
    if(/Có thể cân nhắc|Cân nhắc/i.test(x)) return {tone:'warn', short:'Cân nhắc', title:'Có thể thực hiện, nhưng nên kiểm kỹ trước việc quan trọng', meaning:'Tín hiệu hiện tại không xấu rõ rệt nhưng cũng chưa đủ mạnh để gọi là thuận.'};
    ...
    if(/^Ưu tiên$/i.test(x)) return {tone:'good', short:'Ưu tiên', title:'Đây là một lựa chọn nên ưu tiên cho việc đang xem', meaning:'Lớp sự kiện và lớp cá nhân đang cùng ủng hộ lựa chọn này trong phạm vi quy tắc đã nghiệm thu.'};
    if(/Trung tính|Cân bằng/i.test(x)) return {tone:'neutral', short:'Cân bằng', title:'Thời điểm này tương đối cân bằng', meaning:'Chưa có tín hiệu đủ mạnh để gọi là thuận hay nghịch. Có thể tiếp tục việc thường ngày; việc quan trọng nên kiểm riêng.'};
    return {tone:'neutral', short:'Chưa rõ', title:'Chưa có tín hiệu đủ rõ để kết luận mạnh', meaning:'Ứng dụng chưa có đủ căn cứ để đưa ra kết luận mạnh ở lớp hiện tại.'};
  }
  ...
  function actionGuide(raw){
    ...
    if(p.tone==='warn') return {
      ...
      caution:['Hạn chế quyết định vội khi chưa kiểm đủ thông tin.','Không suy kết luận chung thành dự đoán riêng về tiền bạc hay quan hệ.']
    };
  ...
  function addPlainStyles(){
    ...
    s.textContent=`
      ...
      .home-plain-why{display:block;margin-top:7px;font-size:12px;font-weight:800;color:var(--teal,#0f766e)}
      @media(max-width:640px){.plain-guide,.plain-domains{grid-template-columns:1fr}}
    `;document.head.appendChild(s);
  }

  badgeClass=function(label=''){
    const p=plainStatus(label);return p.tone==='bad'?'bad':p.tone==='good'?'good':p.tone==='warn'?'warn':'neutral';
  };
  ...
  renderTodayA=function(d,p){
    const s=d.don_gian||{},deep=d.chuyen_sau||{},ps=plainStatus(s.tom_tat);const dateText=d.ngay?String(d.ngay).split('-').reverse().join('/'):'Hôm nay';
  ...
  renderCalendar=async function(){
    ...
    document.querySelectorAll('.day-state-text').forEach(el=>{el.textContent=plainStatus(el.textContent).short});
    document.querySelectorAll('.calendar-list-row .result-label').forEach(el=>{el.textContent=plainStatus(el.textContent).short});
  ...
  addPlainStyles();
  window.addEventListener('load',()=>setTimeout(()=>{try{if(currentProfile)loadHomeDashboard();if($('screen-calendar')?.classList.contains('active'))renderCalendar()}catch{}},850));
})();
