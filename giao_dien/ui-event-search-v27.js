// V2.7 — Event Search hoàn chỉnh; V2.8 bổ sung giải thích confidence dựa trên bằng chứng.
(function(){
  window.TU_BINH_EVENT_SEARCH_UI_VERSION = '2.8';
  ...
  function confidenceBasisHtml(r){
    const basis=Array.isArray(r?.confidence_basis)?r.confidence_basis:[];
    if(!basis.length)return '';
    return `<div class="v28-confidence-basis"><b>Vì sao mức căn cứ là ${esc(r?.confidence_state||'Chưa đủ căn cứ')}?</b><ul>${basis.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div>`;
  }
  function traceHtml(r){
    const rules=Array.isArray(r?.rules)?r.rules:[], sources=Array.isArray(r?.sources)?r.sources:[];
    return `<details class="v27-trace"><summary>Vì sao ngày này được xếp như vậy?</summary><p>${esc(r?.plain_explanation||whyText(r))}</p><p class="muted">${esc(whyText(r))}</p>${confidenceBasisHtml(r)}${rules.length?`<p><b>Rule:</b> ${rules.map(esc).join(' · ')}</p>`:''}${sources.length?`<p><b>Nguồn:</b> ${sources.map(esc).join(' · ')}</p>`:''}<p class="muted">Trực ${esc(r?.technical?.truc||'—')} · phạm vi ${esc(r?.technical?.coverage||'—')}</p></details>`;
  }
  ...
  renderCalendar=async function(){
    ...
    try{
      ...
      renderCalendarList();
      const detail=$('calendar-detail');if(detail)detail.innerHTML=`<div class="v27-calendar-note">Lịch đang đánh giá theo đúng loại việc đã chọn bằng cùng engine Event Search. Mức căn cứ được tính riêng từ chất lượng bằng chứng. Chọn một ngày để xem lý do.</div>`;
