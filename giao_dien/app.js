...
function ordinalMeter(label){const cls=badgeClass(label);return `<div class="ordinal-meter ${cls}"><div class="meter-arc"></div><strong>${esc(label||'Đang đánh giá')}</strong><small>0.4 · đúng phương pháp</small></div>`}
...
async function loadHomeDashboard(){
 ...
 try{const d=await post('/api/stateless/dashboard',{profile:p});const pos=d.vi_tri||{};if(cycle)cycle.textContent=`Đại vận ${pos.dai_van?.tru||'—'} · ${pos.nam_hien_tai?.vi||'—'} · ${pos.thang_hien_tai?.vi||'—'}`;
  if(monthEl){const m=d.thang||{},s=m.don_gian||{},dg=s.dien_giai||{};monthEl.classList.remove('loading-card');monthEl.innerHTML=`<button class="home-card-click" onclick="openQuestion('month')"><div class="home-card-head"><div><small>THÁNG NÀY CỦA TÔI</small><b>${esc(m.chuyen_sau?.thang?.tru?.vi||'Tháng hiện tại')}</b></div><span>›</span></div><div class="home-month-body">${ordinalMeter(s.tom_tat)}<div class="home-points"><p><i class="neutral-dot"></i><b>Công việc:</b> ${esc(dg.cong_viec||'Đang đọc cấu trúc')}</p><p><i class="neutral-dot"></i><b>Tài chính:</b> ${esc(dg.tai_chinh||'Chưa đủ căn cứ')}</p><p><i class="neutral-dot"></i><b>Quan hệ:</b> ${esc(dg.quan_he||'Chưa đủ căn cứ')}</p><p><i class="neutral-dot"></i><b>Việc lớn:</b> ${esc(dg.viec_lon||'Chưa đủ căn cứ')}</p></div></div></button>`}
  if(todayEl){const t=d.hom_nay||{},s=t.don_gian||{},dg=s.dien_giai||{};todayEl.classList.remove('loading-card');todayEl.innerHTML=`<button class="home-card-click" onclick="openQuestion('today')"><div class="home-card-head"><div><small>HÔM NAY THẾ NÀO?</small><b>${esc(dg.headline||s.tom_tat)}</b></div><span>›</span></div><div class="today-compact"><span class="today-check neutral">i</span><div><p><b>Cấu trúc:</b> ${esc(dg.cong_viec||s.vi_sao||'Đang đọc cấu trúc')}</p><p><b>Quan hệ:</b> ${esc(dg.quan_he||'Chưa đủ căn cứ')}</p>${t.so_sanh_lien_ke?.ngay_mai?`<small>${esc(t.so_sanh_lien_ke.ngay_mai.so_sanh)}</small>`:''}</div></div></button>`}
