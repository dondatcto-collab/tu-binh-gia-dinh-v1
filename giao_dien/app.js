...
function friendlyError(status,d={}){const code=d.error_code?` (${d.error_code})`:'';if(status===422)return`Hồ sơ đang dùng dữ liệu từ phiên bản cũ hoặc thiếu trường bắt buộc. Hãy mở Hồ sơ → Sửa → Lưu lại.${code}`;if(status===408||status===429||status>=500)return`Máy chủ tính toán tạm thời chưa phản hồi ổn định. Hãy thử lại sau vài giây.${code}`;return`${d.detail||'Yêu cầu chưa thực hiện được.'}${code}`}
...
async function loadHomeDashboard(){
 ...
 const [m,t,pos]=await Promise.allSettled([post('/api/stateless/thang-nay',{profile:p}),post('/api/stateless/hom-nay',{profile:p}),post('/api/stateless/toi-dang-o-dau',{profile:p})]);
 if(pos.status==='fulfilled'&&cycle){const d=pos.value;cycle.textContent=`Đại vận ${d.dai_van?.tru||'—'} · ${d.nam_hien_tai?.vi||'—'} · ${d.thang_hien_tai?.vi||'—'}`}
 if(m.status==='fulfilled'&&monthEl){const d=m.value,s=d.don_gian;monthEl.classList.remove('loading-card');monthEl.innerHTML=`<button class="home-card-click" onclick="openQuestion('month')"><div class="home-card-head"><div><small>THÁNG NÀY CỦA TÔI</small><b>${esc(d.chuyen_sau?.thang?.tru?.vi||'Tháng hiện tại')}</b></div><span>›</span></div><div class="home-month-body">${ordinalMeter(s.tom_tat)}<div class="home-points"><p><i class="good-dot"></i>${esc((s.nen_lam||[])[0]||'Giữ nhịp ổn định')}</p><p><i class="warn-dot"></i>${esc((s.can_nhac||[])[0]||'Theo dõi điều kiện thực tế')}</p><p><i class="neutral-dot"></i>${esc((s.khong_uu_tien||[])[0]||'Không có cấm tuyệt đối')}</p></div></div></button>`}
 else if(monthEl)monthEl.innerHTML='<div class="notice danger">Chưa tải được tổng quan tháng. Chạm lại sau.</div>';
 if(t.status==='fulfilled'&&todayEl){const d=t.value,s=d.don_gian;todayEl.classList.remove('loading-card');const good=(d.gio_trong_ngay||[]).filter(x=>/Phù hợp/.test(x.nhan)).slice(0,3);todayEl.innerHTML=`<button class="home-card-click" onclick="openQuestion('today')"><div class="home-card-head"><div><small>HÔM NAY THẾ NÀO?</small><b class="${badgeClass(s.tom_tat)}-text">${esc(s.tom_tat)}</b></div><span>›</span></div><div class="today-compact"><span class="today-check">✓</span><div><p>${esc(s.vi_sao||'Kết luận theo lớp V1-basic có truy nguồn.')}</p>${good.length?`<small>Giờ phù hợp: ${good.map(x=>esc(x.chi_vi)).join(', ')}</small>`:''}</div></div></button>`}
 else if(todayEl)todayEl.innerHTML='<div class="notice danger">Chưa tải được kết quả hôm nay. Chạm lại sau.</div>';
}
...
function hoursBlock(d){const arr=d.gio_trong_ngay||[];const good=arr.filter(x=>/Phù hợp/.test(x.nhan)).slice(0,4);const caution=arr.filter(x=>/Cân nhắc/.test(x.nhan)).slice(0,4);return `<div class="hours-card"><h4>GIỜ PHÙ HỢP TRONG NGÀY</h4><div class="hour-grid good-hours">${good.map(x=>`<span><b>${esc(x.khoang_gio)}</b><small>${esc(x.chi_vi)}</small></span>`).join('')||'<span class="muted">Chưa có giờ nổi bật</span>'}</div><h4>GIỜ NÊN CÂN NHẮC</h4><div class="hour-grid bad-hours">${caution.map(x=>`<span><b>${esc(x.khoang_gio)}</b><small>${esc(x.chi_vi)}</small></span>`).join('')||'<span class="muted">Không có cảnh báo giờ đặc biệt</span>'}</div></div>`}
function renderTodayA(d,p){const s=d.don_gian,deep=d.chuyen_sau;return `<div class="today-a-hero ${badgeClass(s.tom_tat)}"><div><small>TỔNG QUAN HÔM NAY</small><h2>${esc(s.tom_tat)}</h2><p>${esc(s.vi_sao||'Kết luận theo lớp V1-basic có truy nguồn.')}</p></div><div class="ordinal-disc"><span>V1</span><b>BASIC</b></div></div>${adviceHtml(s)}${hoursBlock(d)}<div class="card why-summary"><h3>TẠI SAO HÔM NAY NHƯ VẬY?</h3>${fusionHtml(deep,true)}<p class="muted">Kết luận hợp lưu Đại vận → Năm → Tháng → Ngày. Bấm dưới đây để xem rule và nguồn.</p><button class="btn full-btn" onclick="loadWhy('day')">Xem chi tiết</button><div id="why-box"></div></div>`}
function renderMonthB(d,p){const s=d.don_gian,deep=d.chuyen_sau;const positives=s.nen_lam||[];const challenges=[...(s.can_nhac||[]),...(s.khong_uu_tien||[])];return `<div class="month-b-timeline">${fusionHtml(deep,false)}</div><div class="month-b-summary"><div class="month-compass">◈</div><div><small>ĐÁNH GIÁ THÁNG</small><h2>${esc(s.tom_tat)}</h2><p>${esc(s.vi_sao||'Kết luận theo lớp V1-basic có truy nguồn.')}</p></div></div><div class="month-dual"><div class="month-panel good"><h3>✓ THUẬN LỢI</h3>${positives.slice(0,5).map(x=>`<p>• ${esc(x)}</p>`).join('')||'<p>• Giữ nhịp ổn định.</p>'}</div><div class="month-panel warn"><h3>! THÁCH THỨC</h3>${challenges.slice(0,5).map(x=>`<p>• ${esc(x)}</p>`).join('')||'<p>• Theo dõi điều kiện thực tế.</p>'}</div></div><div class="card month-focus"><h3>NÊN TẬP TRUNG</h3><div class="focus-chips">${positives.slice(0,4).map((x,i)=>`<span><i>${['◎','▤','♧','◈'][i%4]}</i>${esc(x)}</span>`).join('')||'<span><i>◎</i>Giữ kế hoạch rõ ràng</span>'}</div></div><button class="btn secondary full-btn" onclick="loadWhy('month')">TẠI SAO? · Xem chuyên sâu</button><div id="why-box"></div>`}
...
async function findDates(){
 ...
 try{
  ...
  const rows=top.map((x,i)=>{const hd=hourData[i];const hours=(hd?.gio_trong_ngay||[]).filter(h=>/Phù hợp/.test(h.nhan)).slice(0,3);return {...x,hours}});
  const first=rows[0];$('work-result').innerHTML=`${d.canh_bao_an_toan?`<div class="notice danger"><b>Lưu ý y tế</b>${esc(d.canh_bao_an_toan)}</div>`:''}${first?`<div class="work-winner"><small>NGÀY ƯU TIÊN #1</small><div class="winner-row"><div class="medal">1</div><div class="grow"><h2>${esc(first.ngay)}</h2><strong>${esc(first.label)}</strong><p>${first.hours.length?'Giờ phù hợp: '+first.hours.map(h=>`${esc(h.chi_vi)} (${esc(h.khoang_gio)})`).join(' · '):'Giờ sẽ xem chi tiết sau khi chọn ngày.'}</p></div><span class="ordinal-badge">V1<br>BASIC</span></div></div>`:''}<div class="card work-other"><h3>DANH SÁCH GỢI Ý KHÁC</h3>${rows.slice(1).map((x,i)=>`<div class="work-rank-row"><div class="rank-no">${i+2}</div><div class="grow"><b>${esc(x.ngay)}</b><strong>${esc(x.label)}</strong><small>Trực ${esc(x.truc||'—')} · ${esc(x.personal_relation?.nhan||'')}</small>${x.hours.length?`<p>Giờ phù hợp: ${x.hours.map(h=>esc(h.chi_vi)).join(', ')}</p>`:''}</div><span>›</span></div>`).join('')||'<div class="muted">Không có gợi ý khác.</div>'}<div class="soft-note">${esc(d.ghi_chu||'')}</div><div class="work-buttons"><button class="btn secondary" onclick="document.querySelector('.work-search-card')?.scrollIntoView({behavior:'smooth'})">Chỉnh sửa tìm kiếm</button><button class="btn" onclick="toggleResearchDays()">Xem chi tiết tất cả</button></div><div id="research-days" class="research-days" hidden>${(d.cac_ngay||[]).map(x=>`<div class="research-row"><b>${x.ngay}</b> · ${esc(x.tru_ngay)} · <span class="result-label ${badgeClass(x.label)}">${esc(x.label)}</span><div class="muted">Trực ${esc(x.truc||'—')} · ${esc(x.personal_relation?.nhan||'')}</div></div>`).join('')}</div></div>`;
...
async function checkConnection(){const el=$('connection-state');try{const h=await api('/api/health');if(el)el.textContent=`Engine ${h.engine_version} · kết nối tốt`;}catch(e){if(el)el.textContent=`Engine chưa phản hồi · ${e.message}`}}
