...
def test_schema_status_locks_foundation_principles():
    s = schema_status()
    assert s["schema_version"] == SCHEMA_VERSION == "2.0-alpha.1"
    assert s["numeric_score"] == NUMERIC_SCORE_STATUS == "LOCKED_OFF"
    assert "day" in s["implemented_scopes"]
    assert "month" in s["implemented_scopes"]
    assert "event_search" in s["implemented_scopes"]
...
def test_personal_caution_is_not_rendered_as_bad_absolute_day():
    ...
    assert any("Việc thường ngày" in out["plain_explanation"] for _ in [0])
...
def test_event_search_keeps_ordinal_ranking_and_no_score():
    raw = {
        "viec": "KY_HOP_DONG",
        "so_ngay_da_quet": 3,
        "xep_hang_status": "ORDINAL_V1_1_PERSONAL",
        "top": [
...
def test_required_v2_result_fields_are_stable():
    ...
    required = {
        "schema_version", "kind", "scope", "domain", "conclusion",
        "plain_explanation", "recommended_actions", "cautions",
        "confidence_state", "event_context", "personal_context",
        "evidence", "rules", "sources", "technical",
