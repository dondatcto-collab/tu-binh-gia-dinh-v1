...
def test_yue_en_locks_all_twelve_month_rows():
    expected={
        "DAN":"BINH","MAO":"DINH","THIN":"CANH","TI":"KY",
        "NGO":"MAU","MUI":"TAN","THAN":"NHAM","DAU":"QUY",
        "TUAT":"CANH","HOI":"AT","TY":"GIAP","SUU":"TAN",
    }
    assert YUE_EN_STEM_BY_MONTH_BRANCH==expected
    for month,stem in expected.items():
        assert yue_en_stem(month)==stem
        assert "月恩" in active_stem_tokens(month,stem)


def test_yue_en_nonmatching_stem_is_not_active():
    assert "月恩" not in active_stem_tokens("DAN","GIAP")


def test_yue_en_positive_support_gate():
    out=evaluate_event_v25(_base(),_personal("SUPPORT","BINH"),chi_thang="DAN",chi_ngay="MAO")
    assert "月恩" in out["matched_yi_tokens"]
...
def test_yue_en_cannot_rescue_jie_sha_caution():
    out=evaluate_event_v25(_base(),_personal("SUPPORT","BINH"),chi_thang="DAN",chi_ngay="HOI")
    assert "月恩" in out["matched_yi_tokens"]
...
def test_hard_block_still_wins_over_yue_en():
    out=evaluate_event_v25(_base("JI"),_personal("SUPPORT","BINH"),chi_thang="DAN",chi_ngay="MAO")
    assert "月恩" in out["matched_yi_tokens"]
...
def test_v30e3_capability_and_schema_are_explicit():
    cap=capability_inventory()
    assert cap["token_count"]==81
    assert cap["active_calculable_count"]==23
    assert cap["pending_calculator_count"]==58
    assert "月恩" in cap["active_tokens"]
    assert cap["coverage"]=="12_TRUC_PLUS_MONTH_BRANCH_11_PLUS_DAY_STEM_3"
    assert cap["extension_version"]=="V3_0E3_YUE_EN"
    assert cap["numeric_score"] is None

    s=v25_schema_overlay({"implemented_scopes":[],"pending_scopes":[],"principles":[]})
    assert s["hiep_ky_v25"]["effective_coverage"]==COVERAGE=="V3_0E3_PARTIAL_12_TRUC_PLUS_MONTH_BRANCH_11_PLUS_DAY_STEM_3"
    assert "hiep_ky_v30e3_yue_en" in s["implemented_scopes"]
    v=s["hiep_ky_v30e3"]
    assert v["activated_token"]=="月恩"
    assert v["calculator"]=="MONTH_BRANCH_DAY_STEM_V30E3"
