...
def test_wu_he_is_exactly_tiger_and_rabbit_days():
    ...
    for branch in ("TY","SUU","THIN","TI","NGO","MUI","THAN","DAU","TUAT","HOI"):
        assert active_day_branch_tokens(branch) == ()


def test_wu_he_invalid_day_branch_fails_closed():
    try:
        active_day_branch_tokens("INVALID")
    except ValueError:
        pass
    else:
        raise AssertionError("invalid Chi must fail closed")


def test_wu_he_event_inventory_is_scoped():
    verified=[x for x in evidence_for_event("KY_HOP_DONG") if x.token=="五合"]
    assert len(verified)==1 and verified[0].polarity=="YI" and verified[0].evidence_status=="VERIFIED"
    provisional=[x for x in evidence_for_event("DAM_PHAN") if x.token=="五合"]
    assert len(provisional)==1 and provisional[0].polarity=="YI" and provisional[0].evidence_status=="PROVISIONAL"
    for event_code in {"KHAI_TRUONG","MUA_TAI_SAN","DONG_THO","NHAP_TRACH","CUOI_HOI","XUAT_HANH","DIEU_TRI","NHAM_CHUC","CAU_TAI","AN_TANG"}:
        assert all(x.token!="五合" for x in evidence_for_event(event_code))
...
def test_wu_he_positive_gate_for_verified_contract_event_without_day_stem():
    ...
    assert "五合" in out["active_hiep_ky_tokens"]
    assert "五合" in out["matched_yi_tokens"]
...
def test_wu_he_cannot_rescue_month_harm_caution():
    out=evaluate_event_v25(_base(),_personal("SUPPORT",None),chi_thang="TI",chi_ngay="DAN")
    assert "五合" in out["matched_yi_tokens"]
    assert "月害" in out["matched_ji_tokens"]
...
def test_hard_block_still_wins_over_wu_he():
    out=evaluate_event_v25(_base("JI"),_personal("SUPPORT",None),chi_thang="THIN",chi_ngay="MAO")
    assert "五合" in out["matched_yi_tokens"]
...
def test_wu_he_does_not_leak_to_unsupported_event():
    out=evaluate_event_v25(_base(event_code="XUAT_HANH"),_personal("SUPPORT",None),chi_thang="THIN",chi_ngay="DAN")
    assert "五合" in out["active_hiep_ky_tokens"]
    assert "五合" not in out["matched_yi_tokens"]


def test_wu_he_provisional_mapping_is_capped():
    out=evaluate_event_v25(_base(event_code="DAM_PHAN",mapping="PROVISIONAL"),_personal("SUPPORT",None),chi_thang="THIN",chi_ngay="DAN")
    assert "五合" in out["matched_yi_tokens"]
    assert out["label"]=="Có thể cân nhắc"
    assert out["decision_authority"]=="EVENT_PROVISIONAL"


def test_v30e8_capability_schema_and_score_are_explicit():
    ...
    assert "五合" in cap["active_tokens"]
    assert token_capability("五合")["calculator"]=="DAY_BRANCH_V30E8"
    ...
    assert "hiep_ky_v30e8_wu_he" in schema["implemented_scopes"]
    v=schema["hiep_ky_v30e8"]
    assert v["activated_token"]=="五合"
    assert v["calculator"]=="DAY_BRANCH_V30E8"
