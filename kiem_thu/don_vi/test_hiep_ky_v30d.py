...
def test_v30d_shi_de_remains_active_after_v30e2():
    cap=capability_inventory(); assert cap["token_count"]==81; assert cap["active_calculable_count"]==22; assert cap["pending_calculator_count"]==59; assert "時徳" in cap["active_tokens"]; assert "月徳" in cap["active_tokens"]; assert "月徳合" in cap["active_tokens"]; assert cap["coverage"]=="12_TRUC_PLUS_MONTH_BRANCH_11_PLUS_DAY_STEM_2"; assert cap["extension_version"]=="V3_0E2_YUE_DE_HE"


def test_v30d_shi_de_can_support_verified_neutral_event():
    out=evaluate_event_v25(_base(),_personal("SUPPORT"),chi_thang="DAN",chi_ngay="NGO"); assert "時徳" in out["active_hiep_ky_tokens"]; assert out["matched_yi_tokens"]==["時徳"]; assert out["matched_ji_tokens"]==[]; assert out["event_signal_v25"]=="FAVORABLE"; assert out["label"]=="Ưu tiên"; assert out["numeric_score"] is None
...
def test_v30d_schema_remains_explicit_after_v30e2():
