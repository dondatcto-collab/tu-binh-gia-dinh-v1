...
def test_v30d_shi_de_remains_active_after_v30e3():
    cap=capability_inventory(); assert cap["token_count"]==81; assert cap["active_calculable_count"]==23; assert cap["pending_calculator_count"]==58; assert "時徳" in cap["active_tokens"]; assert "月徳" in cap["active_tokens"]; assert "月徳合" in cap["active_tokens"]; assert "月恩" in cap["active_tokens"]; assert cap["coverage"]=="12_TRUC_PLUS_MONTH_BRANCH_11_PLUS_DAY_STEM_3"; assert cap["extension_version"]=="V3_0E3_YUE_EN"
...
def test_v30d_schema_remains_explicit_after_v30e3():
