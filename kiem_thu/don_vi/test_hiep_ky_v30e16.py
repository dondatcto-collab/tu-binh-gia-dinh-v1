...
def test_lin_ri_locks_all_twelve_months():
    ...
    for month,day in expected.items(): assert lin_ri_branch(month)==day and active_lin_ri_tokens(month,day)==("臨日",)
...
def test_lin_ri_verified_scope_is_nham_chuc_only():
    ...
    for event_code in ("XUAT_HANH","KHAI_TRUONG","KY_HOP_DONG","CAU_TAI","DIEU_TRI"): assert all(x.token!="臨日" for x in evidence_for_event(event_code))

def test_lin_ri_support_and_hard_block_precedence():
    out=evaluate_event_v25(_base(),_personal(),chi_thang="DAN",chi_ngay="NGO"); assert "臨日" in out["active_hiep_ky_tokens"] and "臨日" in out["matched_yi_tokens"] and out["numeric_score"] is None
    blocked=evaluate_event_v25(_base("JI"),_personal(),chi_thang="DAN",chi_ngay="NGO"); assert blocked["decision_state"]=="BLOCKED" and blocked["hard_block"] is True
...
def test_e16_contract_remains_bound_after_later_releases():
    ...
    status=calculator_status(); assert status["active_tokens"]==("臨日",) and status["numeric_score"] is None
