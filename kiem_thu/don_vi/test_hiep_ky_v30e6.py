...
from loi.quyet_dinh.hiep_ky_season_day_pillar_v30e6 import (
    TIAN_SHE_DAY_PILLAR_BY_MONTH_BRANCH,
    active_season_day_pillar_tokens,
    calculator_status,
    tian_she_day_pillar,
)
...
def test_tian_she_locks_four_seasons_across_all_twelve_month_branches():
    expected={
        "DAN":("MAU","DAN"),"MAO":("MAU","DAN"),"THIN":("MAU","DAN"),
        "TI":("GIAP","NGO"),"NGO":("GIAP","NGO"),"MUI":("GIAP","NGO"),
        "THAN":("MAU","THAN"),"DAU":("MAU","THAN"),"TUAT":("MAU","THAN"),
        "HOI":("GIAP","TY"),"TY":("GIAP","TY"),"SUU":("GIAP","TY"),
    }
    assert TIAN_SHE_DAY_PILLAR_BY_MONTH_BRANCH==expected
    for month,pillar in expected.items():
        assert tian_she_day_pillar(month)==pillar
        assert active_season_day_pillar_tokens(month,*pillar)==("天赦",)


def test_tian_she_requires_full_day_pillar_and_invalid_inputs_fail_closed():
    assert active_season_day_pillar_tokens("DAN","MAU","MAO")==()
    assert active_season_day_pillar_tokens("DAN","GIAP","DAN")==()
...
def test_tian_she_event_inventory_is_yi_only_where_classical_event_lists_it():
    ...
    for event_code in supported:
        rows=[x for x in evidence_for_event(event_code) if x.token=="天赦"]
        assert len(rows)==1
        assert rows[0].polarity=="YI"
    for event_code in {"KHAI_TRUONG","KY_HOP_DONG","MUA_TAI_SAN","CAU_TAI"}:
        assert all(x.token!="天赦" for x in evidence_for_event(event_code))


def test_tian_she_positive_gate_for_verified_medical_event():
    out=evaluate_event_v25(_base(),_personal("SUPPORT","MAU"),chi_thang="THIN",chi_ngay="DAN")
    assert "天赦" in out["matched_yi_tokens"]
...
def test_tian_she_cannot_rescue_month_build_caution():
    out=evaluate_event_v25(_base(event_code="DONG_THO"),_personal("SUPPORT","MAU"),chi_thang="DAN",chi_ngay="DAN")
    assert "天赦" in out["matched_yi_tokens"]
    assert "月建" in out["matched_ji_tokens"]
...
def test_hard_block_still_wins_over_tian_she():
    out=evaluate_event_v25(_base("JI"),_personal("SUPPORT","MAU"),chi_thang="THIN",chi_ngay="DAN")
    assert "天赦" in out["matched_yi_tokens"]
...
def test_tian_she_fails_closed_without_current_stem():
    out=evaluate_event_v25(_base(),_personal("SUPPORT",None),chi_thang="THIN",chi_ngay="DAN")
    assert "天赦" not in out["active_hiep_ky_tokens"]


def test_tian_she_does_not_leak_to_unsupported_opening_event():
    out=evaluate_event_v25(_base(event_code="KHAI_TRUONG"),_personal("SUPPORT","MAU"),chi_thang="THIN",chi_ngay="DAN")
    assert "天赦" in out["active_hiep_ky_tokens"]
    assert "天赦" not in out["matched_yi_tokens"]


def test_provisional_event_cannot_be_promoted_to_absolute_priority_by_tian_she():
    out=evaluate_event_v25(_base(event_code="DAM_PHAN",mapping="PROVISIONAL"),_personal("SUPPORT","MAU"),chi_thang="THIN",chi_ngay="DAN")
    assert "天赦" in out["matched_yi_tokens"]
    assert out["label"]=="Có thể cân nhắc"
    assert out["decision_authority"]=="EVENT_PROVISIONAL"


def test_v30e6_capability_schema_and_score_are_explicit():
    ...
    assert token_capability("天赦")["calculator"]=="SEASON_DAY_PILLAR_V30E6"
    ...
    assert "hiep_ky_v30e6_tian_she" in schema["implemented_scopes"]
    v=schema["hiep_ky_v30e6"]
    assert v["activated_token"]=="天赦"
    assert v["calculator"]=="SEASON_DAY_PILLAR_V30E6"
