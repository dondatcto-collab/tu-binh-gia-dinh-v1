...
from loi.quyet_dinh.hiep_ky_season_branch_v30e7 import (
    TIAN_XI_BRANCH_BY_MONTH_BRANCH,
    active_season_branch_tokens,
    calculator_status,
    tian_xi_branch,
)
...
def test_tian_xi_locks_four_seasons_across_all_twelve_month_branches():
    ...
    for month,day_branch in expected.items():
        assert tian_xi_branch(month)==day_branch
        assert active_season_branch_tokens(month,day_branch)==("天喜",)
...
def test_tian_xi_event_inventory_is_yi_only_where_event_lists_it():
    ...
    for event_code in supported:
        rows=[x for x in evidence_for_event(event_code) if x.token=="天喜"]
        assert len(rows)==1
        assert rows[0].polarity=="YI"
    for event_code in {"KHAI_TRUONG","KY_HOP_DONG","MUA_TAI_SAN","DONG_THO","NHAP_TRACH","DIEU_TRI","CAU_TAI","AN_TANG"}:
        assert all(x.token!="天喜" for x in evidence_for_event(event_code))


def test_tian_xi_positive_gate_for_verified_marriage_event_without_day_stem():
    out=evaluate_event_v25(_base(),_personal("SUPPORT",None),chi_thang="MAO",chi_ngay="NGO")
    assert "天喜" in out["active_hiep_ky_tokens"]
    assert "天喜" in out["matched_yi_tokens"]
...
def test_tian_xi_ji_conflict_month_ngo_day_suu_month_sha_wins():
    out=evaluate_event_v25(_base(),_personal("SUPPORT",None),chi_thang="NGO",chi_ngay="SUU")
    assert "天喜" in out["matched_yi_tokens"]
...
def test_hard_block_still_wins_over_tian_xi():
    out=evaluate_event_v25(_base("JI"),_personal("SUPPORT",None),chi_thang="MAO",chi_ngay="NGO")
    assert "天喜" in out["matched_yi_tokens"]
...
def test_tian_xi_does_not_require_current_stem_but_stem_rules_still_fail_closed():
    ...
    assert "月徳合" not in out["active_hiep_ky_tokens"]
    assert "月恩" not in out["active_hiep_ky_tokens"]


def test_tian_xi_does_not_leak_to_unsupported_opening_event():
    out=evaluate_event_v25(_base(event_code="KHAI_TRUONG"),_personal("SUPPORT",None),chi_thang="MAO",chi_ngay="NGO")
    assert "天喜" in out["active_hiep_ky_tokens"]
    assert "天喜" not in out["matched_yi_tokens"]


def test_provisional_event_cannot_be_promoted_to_absolute_priority_by_tian_xi():
    out=evaluate_event_v25(_base(event_code="DAM_PHAN",mapping="PROVISIONAL"),_personal("SUPPORT",None),chi_thang="MAO",chi_ngay="NGO")
    assert "天喜" in out["matched_yi_tokens"]
...
def test_v30e7_capability_schema_and_score_are_explicit():
    ...
    assert "hiep_ky_v30e7_tian_xi" in schema["implemented_scopes"]
    v=schema["hiep_ky_v30e7"]
    assert v["activated_token"]=="天喜"
    assert v["calculator"]=="SEASON_DAY_BRANCH_V30E7"
