...
from loi.quyet_dinh.hiep_ky_stem_v30e import (
...
def test_yue_de_locks_all_twelve_month_branch_to_day_stem_rows():
    ...
    for month_branch, day_stem in expected.items():
        assert yue_de_stem(month_branch)==day_stem
        assert active_stem_tokens(month_branch,day_stem)==("月徳",)
...
def test_yue_de_negative_and_invalid_inputs_fail_closed():
    ...
    for args in (("INVALID","BINH"),("DAN","INVALID")):
        try:
            active_stem_tokens(*args)
        except ValueError:
            pass
        else:
            raise AssertionError("invalid Can/Chi must fail closed")


def test_v30e1_capability_moves_exactly_yue_de_to_active():
    cap=capability_inventory()
    assert cap["token_count"]==81
    assert cap["active_calculable_count"]==21
    assert cap["pending_calculator_count"]==60
    assert "月徳" in cap["active_tokens"]
    assert token_capability("月徳")["calculator"]=="MONTH_BRANCH_DAY_STEM_V30E1"
    assert token_capability("月徳合")["calculator_status"]=="PENDING_CALCULATOR"


def test_yue_de_can_support_verified_neutral_event():
    # Tháng Dần + ngày Bính Thìn: 月徳 active, không có month-branch JI đang kích hoạt.
...
def test_ji_wins_when_yue_de_overlaps_zai_sha():
    # Tháng Dần + ngày Bính Tý: 月徳 (宜) + 災煞 (忌) cùng có; Xuất hành ghi cả hai.
    out=evaluate_event_v25(_base(),_personal("SUPPORT"),chi_thang="DAN",chi_ngay="TY",can_ngay="BINH")
    assert "月徳" in out["matched_yi_tokens"]
    assert "災煞" in out["matched_ji_tokens"]
...
def test_existing_hard_block_still_wins_over_yue_de():
    ...
    assert out["label"]=="Bị chặn"


def test_missing_day_stem_never_implicitly_activates_yue_de():
    out=evaluate_event_v25(_base(),_personal("SUPPORT"),chi_thang="DAN",chi_ngay="THIN")
    assert "月徳" not in out["active_hiep_ky_tokens"]


def test_v30e1_schema_and_calculator_are_explicit_no_score():
    calc=calculator_status()
    assert calc["calculator"]=="MONTH_BRANCH_DAY_STEM_V30E1"
    assert calc["active_tokens"]==("月徳",)
    assert calc["numeric_score"] is None
    s=v25_schema_overlay({"implemented_scopes":[],"pending_scopes":[],"principles":[]})
    assert "hiep_ky_v30e1_yue_de" in s["implemented_scopes"]
    v=s["hiep_ky_v30e1"]
