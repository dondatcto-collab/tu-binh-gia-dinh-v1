...
def test_yue_de_locks_all_twelve_month_branch_to_day_stem_rows():
    ...
    for month_branch,day_stem in expected.items(): assert yue_de_stem(month_branch)==day_stem; assert "月徳" in active_stem_tokens(month_branch,day_stem)
...
def test_v30e1_yue_de_remains_active_after_v30e9():
    cap=capability_inventory(); assert cap["token_count"]==81; assert cap["active_calculable_count"]==29; assert cap["pending_calculator_count"]==52; assert "月徳" in cap["active_tokens"]; assert {"天喜","五合","天醫"}.issubset(set(cap["active_tokens"])); assert token_capability("月徳")["calculator"]=="MONTH_BRANCH_DAY_STEM_V30E3"


def test_yue_de_can_support_verified_neutral_event():
    out=evaluate_event_v25(_base(),_personal("SUPPORT"),chi_thang="DAN",chi_ngay="THIN",can_ngay="BINH"); assert "月徳" in out["active_hiep_ky_tokens"]; assert "月徳" in out["matched_yi_tokens"]; assert out["matched_ji_tokens"]==[]; assert out["event_signal_v25"]=="FAVORABLE"; assert out["label"]=="Ưu tiên"; assert out["numeric_score"] is None


def test_yue_de_uses_typed_current_stem_contract_for_live_pipeline():
    out=evaluate_event_v25(_base(),_personal("SUPPORT","BINH"),chi_thang="DAN",chi_ngay="THIN"); assert "月徳" in out["active_hiep_ky_tokens"]; assert "月徳" in out["matched_yi_tokens"]; assert out["personal_v1_1"]["current_stem"]=="BINH"


def test_technical_text_is_not_parsed_for_day_stem_anymore():
    out=evaluate_event_v25(_base(),_personal("SUPPORT",None,["Can Bính đối với Nhật chủ là kiểm thử"]),chi_thang="DAN",chi_ngay="THIN"); assert "月徳" not in out["active_hiep_ky_tokens"]; assert "月徳" not in out["matched_yi_tokens"]


def test_invalid_typed_current_stem_fails_closed():
    out=evaluate_event_v25(_base(),_personal("SUPPORT","INVALID"),chi_thang="DAN",chi_ngay="THIN"); assert "月徳" not in out["active_hiep_ky_tokens"]; assert "月徳" not in out["matched_yi_tokens"]


def test_explicit_day_stem_remains_backward_compatible_and_takes_precedence():
    out=evaluate_event_v25(_base(),_personal("SUPPORT","GIAP"),chi_thang="DAN",chi_ngay="THIN",can_ngay="BINH"); assert "月徳" in out["active_hiep_ky_tokens"]; assert "月徳" in out["matched_yi_tokens"]


def test_ji_wins_when_yue_de_overlaps_zai_sha():
    out=evaluate_event_v25(_base(),_personal("SUPPORT"),chi_thang="DAN",chi_ngay="TY",can_ngay="BINH"); assert "月徳" in out["matched_yi_tokens"]; assert "災煞" in out["matched_ji_tokens"]; assert out["event_signal_v25"]=="CAUTION"; assert out["label"]=="Không ưu tiên"; assert out["hard_block"] is False; assert out["numeric_score"] is None


def test_existing_hard_block_still_wins_over_yue_de():
    out=evaluate_event_v25(_base("JI"),_personal("SUPPORT"),chi_thang="DAN",chi_ngay="THIN",can_ngay="BINH"); assert "月徳" in out["matched_yi_tokens"]; assert out["hard_block"] is True; assert out["decision_state"]=="BLOCKED"; assert out["label"]=="Bị chặn"
...
def test_v30e1_schema_remains_explicit_after_v30e9():
