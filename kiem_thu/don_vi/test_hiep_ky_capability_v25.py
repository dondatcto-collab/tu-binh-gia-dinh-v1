from loi.quyet_dinh.hiep_ky_capability_v25 import MONTH_BRANCH_TOKENS,TRUC_TOKEN_TO_CODE,capability_inventory,token_capability
...
def test_v30d_exact_eleven_month_branch_tokens_are_active():
    assert MONTH_BRANCH_TOKENS==frozenset({"月建","月破","三合","六合","月害","月刑","劫煞","災煞","月煞","月厭","時徳"})
    for token in MONTH_BRANCH_TOKENS:
        row=token_capability(token); assert row["calculator_status"]=="ACTIVE_CALCULABLE"; assert row["calculator"]=="MONTH_BRANCH_RELATIONS_V25_V30D"; assert row["normalized_code"]==token


def test_named_star_without_calculator_stays_pending():
    for token in ("天徳","月徳","天願","天醫","月恩","四相"):
...
def test_inventory_claims_only_partial_decision_expansion():
    s=capability_inventory(); assert s["active_calculable_count"]==20; assert s["pending_calculator_count"]==61; assert s["coverage"]=="12_TRUC_PLUS_MONTH_BRANCH_11"; assert s["extension_version"]=="V3_0D_SHI_DE"; assert s["numeric_score"] is None
