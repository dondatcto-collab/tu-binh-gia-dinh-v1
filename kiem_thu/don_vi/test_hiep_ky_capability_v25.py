...
def test_v30e2_opens_exactly_two_day_stem_tokens():
    assert MONTH_BRANCH_DAY_STEM_TOKENS==frozenset({"月徳","月徳合"})
    for token in MONTH_BRANCH_DAY_STEM_TOKENS:
        row=token_capability(token); assert row["calculator_status"]=="ACTIVE_CALCULABLE"; assert row["calculator"]=="MONTH_BRANCH_DAY_STEM_V30E2"; assert row["normalized_code"]==token


def test_other_named_stars_without_calculator_stay_pending():
    for token in ("天徳","天願","天醫","月恩","四相"):
...
def test_inventory_claims_only_partial_decision_expansion():
    s=capability_inventory(); assert s["token_count"]==81; assert s["active_calculable_count"]==22; assert s["pending_calculator_count"]==59; assert s["coverage"]=="12_TRUC_PLUS_MONTH_BRANCH_11_PLUS_DAY_STEM_2"; assert s["extension_version"]=="V3_0E2_YUE_DE_HE"; assert s["numeric_score"] is None; assert s["numeric_score_status"]=="LOCKED_OFF"
