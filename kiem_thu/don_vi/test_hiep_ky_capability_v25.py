...
def test_v30a_exact_six_month_branch_tokens_are_active():
    assert MONTH_BRANCH_TOKENS == frozenset({"月建", "月破", "三合", "六合", "月害", "月刑"})
    for token in MONTH_BRANCH_TOKENS:
        row = token_capability(token)
        assert row["calculator_status"] == "ACTIVE_CALCULABLE"
        assert row["calculator"] == "MONTH_BRANCH_RELATIONS_V25_V30A"
        assert row["normalized_code"] == token


def test_named_star_without_calculator_stays_pending():
    for token in ("天徳", "月徳", "天願", "劫煞", "災煞", "月煞"):
        row = token_capability(token)
...
def test_inventory_claims_only_partial_decision_expansion():
    ...
    assert status["coverage"] == "12_TRUC_PLUS_MONTH_BRANCH_6"
    assert status["extension_version"] == "V3_0A_YUE_XING"
