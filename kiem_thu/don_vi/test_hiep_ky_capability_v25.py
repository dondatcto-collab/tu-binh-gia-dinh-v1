from loi.quyet_dinh.hiep_ky_capability_v25 import (
    ...
    MONTH_BRANCH_DAY_STEM_TOKENS, MONTH_BRANCH_TOKENS, MONTH_LIN_RI_TOKENS,
...
def test_v30d_eleven_month_branch_tokens_remain_active():
    ...
    for token in MONTH_BRANCH_TOKENS:
        row = token_capability(token)
        assert row["calculator_status"] == "ACTIVE_CALCULABLE"
        assert row["calculator"] == "MONTH_BRANCH_RELATIONS_V25_V30D"
...
def test_typed_calculator_groups_are_exact():
    ...
    assert SEASON_GUAN_RI_TOKENS == frozenset({"官日"})
    assert SEASON_XIANG_RI_TOKENS == frozenset({"相日"})
...
def test_known_calculators_remain_bound_to_their_tokens():
    expected = {
        ...
        "天喜":"SEASON_DAY_BRANCH_V30E7", "五合":"DAY_BRANCH_V30E8", "天醫":"MONTH_BRANCH_DAY_BRANCH_V30E9",
        "解神":"MONTH_BRANCH_DAY_BRANCH_V30E10_GIAI_THAN", "五富":"MONTH_BRANCH_DAY_BRANCH_V30E11_WU_FU",
        ...
        "臨日":"MONTH_BRANCH_DAY_BRANCH_V30E16_LIN_RI",
    }
    for token, calculator in expected.items():
        row = token_capability(token)
        assert row["calculator_status"] == "ACTIVE_CALCULABLE"
        assert row["calculator"] == calculator
...
def test_current_global_capability_is_checked_only_here():
    ...
    assert s["active_calculable_count"] == 36
    assert s["pending_calculator_count"] == 45
    assert s["extension_version"] == "V3_0E16_LIN_RI"
