...
def test_v30e1_opens_exactly_yue_de_day_stem_token():
    assert MONTH_BRANCH_DAY_STEM_TOKENS == frozenset({"月徳"})
    row = token_capability("月徳")
    assert row["calculator_status"] == "ACTIVE_CALCULABLE"
    assert row["calculator"] == "MONTH_BRANCH_DAY_STEM_V30E1"
    assert row["normalized_code"] == "月徳"


def test_other_named_stars_without_calculator_stay_pending():
    for token in ("天徳","月徳合","天願","天醫","月恩","四相"):
        row = token_capability(token)
...
def test_inventory_claims_only_partial_decision_expansion():
    ...
    assert s["active_calculable_count"] == 21
    assert s["pending_calculator_count"] == 60
    assert s["coverage"] == "12_TRUC_PLUS_MONTH_BRANCH_11_PLUS_DAY_STEM_1"
    assert s["extension_version"] == "V3_0E1_YUE_DE"
