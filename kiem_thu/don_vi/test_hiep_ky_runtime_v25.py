...
def test_hard_block_v1_still_wins_everything():
    ...
    assert r["hard_block"] is True; assert r["decision_state"]=="BLOCKED"; assert r["label"]=="Bị chặn"; assert r["rank_group"]==9
...
def test_new_yi_token_can_support_neutral_event_without_score():
    ...
    assert r["matched_yi_tokens"]==["六合"]; assert r["event_signal_v25"]=="FAVORABLE"; assert r["label"]=="Ưu tiên"; assert r["score"] is None
...
def test_v30a_yue_hai_and_yue_xing_both_survive_as_caution_evidence():
    ...
    assert set(r["matched_ji_tokens"])=={"月害","月刑"}; assert r["event_signal_v25"]=="CAUTION"; assert r["hard_block"] is False; assert r["label"]=="Không ưu tiên"


def test_v30c_yue_yan_is_caution_not_hard_block():
    r=evaluate_event_v25(_base(),_personal("SUPPORT"),chi_thang="DAN",chi_ngay="TUAT")
    assert "月厭" in r["matched_ji_tokens"]; assert r["event_signal_v25"]=="CAUTION"; assert r["hard_block"] is False


def test_personal_cannot_rescue_event_caution():
    r=evaluate_event_v25(_base(),_personal("SUPPORT"),chi_thang="TY",chi_ngay="MAO")
    assert r["label"]=="Không ưu tiên"; assert r["rank_group"]==4
...
def test_provisional_mapping_cannot_be_promoted_to_priority_by_new_signal():
    ...
    assert "六合" in r["matched_yi_tokens"]; assert r["label"]=="Có thể cân nhắc"; assert r["decision_authority"]=="EVENT_PROVISIONAL"
...
def test_runtime_is_traceable_and_coverage_is_explicitly_partial():
    ...
    assert r["coverage"]==COVERAGE=="V3_0E1_PARTIAL_12_TRUC_PLUS_MONTH_BRANCH_11_PLUS_DAY_STEM_1"
    assert r["hiep_ky_extension"]=="V3_0E1_YUE_DE"


def test_day_stem_rule_fails_closed_when_current_stem_is_unknown():
    r=evaluate_event_v25(_base(event_code="XUAT_HANH"),_personal("SUPPORT"),chi_thang="DAN",chi_ngay="THIN")
    assert "月徳" not in r["active_hiep_ky_tokens"]
