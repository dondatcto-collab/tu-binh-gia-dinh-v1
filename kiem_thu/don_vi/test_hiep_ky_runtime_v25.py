...
def test_v30a_yue_xing_alone_is_caution_not_hard_block():
    r=evaluate_event_v25(_base(),_personal("SUPPORT"),chi_thang="TY",chi_ngay="MAO"); assert r["matched_ji_tokens"]==["月刑"]; assert r["hard_block"] is False; ev=r["matched_evidence"][0]; assert ev["source_id"]=="SRC-HK-QD-V11-WIKISOURCE"

def test_v30c_yue_yan_is_caution_not_hard_block():
    r=evaluate_event_v25(_base(),_personal("SUPPORT"),chi_thang="DAN",chi_ngay="TUAT"); assert "月厭" in r["matched_ji_tokens"]; assert r["event_signal_v25"]=="CAUTION"; assert r["hard_block"] is False; assert r["label"]=="Không ưu tiên"
...
def test_runtime_is_traceable_and_coverage_is_explicitly_partial():
    r=evaluate_event_v25(_base(),_personal(),chi_thang="DAN",chi_ngay="HOI"); assert r["matched_evidence"]; ev=r["matched_evidence"][0]; assert ev["rule_id"] and ev["source_id"] and ev["source_location"]; assert ev["decision_status"]=="ACTIVE"; assert r["coverage"]==COVERAGE=="V3_0D_PARTIAL_12_TRUC_PLUS_MONTH_BRANCH_11"; assert r["hiep_ky_extension"]=="V3_0D_SHI_DE"
