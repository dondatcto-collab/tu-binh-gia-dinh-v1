...
def test_schema_overlay_declares_partial_not_full_classical_coverage():
    ...
    assert "hiep_ky_v30c_yue_yan" in out["implemented_scopes"]
    assert "full_classical_hiep_ky" in out["pending_scopes"]
    assert out["hiep_ky_v25"]["full_classical_claim"] is False
    assert out["hiep_ky_v30a"]["extension_version"] == "V3_0A_YUE_XING"
    assert out["hiep_ky_v30b"]["extension_version"] == "V3_0B_SAT_TRIO"
    v30c = out["hiep_ky_v30c"]
    assert v30c["extension_version"] == "V3_0C_YUE_YAN"
    assert v30c["activated_token"] == "月厭"
    assert v30c["activated_token_vi"] == "Nguyệt Yếm"
    assert v30c["decision_effect"] == "CAUTION_ONLY"
    assert v30c["creates_hard_block"] is False
    assert v30c["full_classical_claim"] is False
    assert v30c["numeric_score"] is None
...
def test_event_search_result_keeps_rules_sources_and_no_score():
    raw = {
        ...
        "top": [{
            ...
            "coverage": "V3_0C_PARTIAL_12_TRUC_PLUS_MONTH_BRANCH_10",
            "hiep_ky_extension": "V3_0C_YUE_YAN",
            "rule_ids": ["HK25-X"],
            "source_ids": ["SRC-HK-QD-V11-WIKISOURCE"],
            "matched_evidence": [{"rule_id": "HK25-X", "source_id": "SRC-HK-QD-V11-WIKISOURCE"}],
            "active_hiep_ky_tokens": ["月厭"],
            "matched_yi_tokens": [],
            "matched_ji_tokens": ["月厭"],
    ...
    assert out["hiep_ky_extension"] == "V3_0C_YUE_YAN"
    ...
    assert item["technical"]["hiep_ky_extension"] == "V3_0C_YUE_YAN"
