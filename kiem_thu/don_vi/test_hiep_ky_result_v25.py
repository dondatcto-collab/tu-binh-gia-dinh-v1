...
def test_schema_overlay_declares_partial_not_full_classical_coverage():
    ...
    e1=out["hiep_ky_v30e1"]; assert e1["activated_token"]=="月徳"; assert e1["calculator"]=="MONTH_BRANCH_DAY_STEM_V30E2"
    e2=out["hiep_ky_v30e2"]; assert e2["extension_version"]=="V3_0E2_YUE_DE_HE"; assert e2["activated_token"]=="月徳合"; assert e2["activated_token_vi"]=="Nguyệt Đức Hợp"; assert e2["calculator"]=="MONTH_BRANCH_DAY_STEM_V30E2"; assert e2["decision_effect"]=="FAVORABLE_SUPPORT_ONLY"; assert e2["creates_hard_block"] is False; assert e2["full_classical_claim"] is False; assert e2["numeric_score"] is None; assert out["numeric_score"]=="LOCKED_OFF"


def test_event_search_result_keeps_rules_sources_and_no_score():
    raw={"viec":"KY_HOP_DONG","so_ngay_da_quet":1,"xep_hang_status":"ORDINAL_V25_HARD_BLOCK_EVENT_PERSONAL","top":[{"ngay":"2026-09-01","label":"Không ưu tiên","decision_state":"CAUTION","hard_block":False,"rank_group":4,"event_state":"CAUTION","personal_v1_1":{},"reasons":["reason"],"mapping_status":"VERIFIED","coverage":"V3_0E2_PARTIAL_12_TRUC_PLUS_MONTH_BRANCH_11_PLUS_DAY_STEM_2","hiep_ky_extension":"V3_0E2_YUE_DE_HE","rule_ids":["HK25-X"],"source_ids":["SRC-HK-QD-V11-WIKISOURCE"],"matched_evidence":[{"rule_id":"HK25-X","source_id":"SRC-HK-QD-V11-WIKISOURCE"}],"active_hiep_ky_tokens":["月厭"],"matched_yi_tokens":[],"matched_ji_tokens":["月厭"],"decision_authority":"EVENT","event_state_v1":"NEUTRAL","event_signal_v25":"CAUTION"}]}
    out=event_search_v25(raw); item=out["results"][0]
    assert out["hiep_ky_extension"]=="V3_0E2_YUE_DE_HE"; assert out["numeric_score"] is None; assert item["rules"]==["HK25-X"]; assert item["sources"]==["SRC-HK-QD-V11-WIKISOURCE"]; assert item["technical"]["matched_ji_tokens"]==["月厭"]; assert item["technical"]["hiep_ky_extension"]=="V3_0E2_YUE_DE_HE"; assert item["numeric_score"] is None
