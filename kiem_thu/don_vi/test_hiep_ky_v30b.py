...
def _neutral_personal(state="NEUTRAL"): return {"state":state,"rule_ids":[],"source_ids":[]}


def test_v30b_tokens_remain_active_after_v30d():
    cap=capability_inventory(); assert cap["token_count"]==81; assert cap["active_calculable_count"]==20; assert cap["pending_calculator_count"]==61; assert {"劫煞","災煞","月煞"}.issubset(set(cap["active_tokens"])); assert cap["coverage"]=="12_TRUC_PLUS_MONTH_BRANCH_11"; assert cap["extension_version"]=="V3_0D_SHI_DE"; assert cap["numeric_score"] is None
...
def test_v30b_schema_remains_explicit_after_v30d():
