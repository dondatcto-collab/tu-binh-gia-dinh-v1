...
def test_v30e2_capability_and_schema_are_explicit():
    cap=capability_inventory(); assert cap["active_calculable_count"]==22; assert cap["pending_calculator_count"]==59; assert "月徳合" in cap["active_tokens"]
    s=v25_schema_overlay({"implemented_scopes":[],"pending_scopes":[],"principles":[]}); assert s["hiep_ky_v25"]["effective_coverage"]==COVERAGE=="V3_0E2_PARTIAL_12_TRUC_PLUS_MONTH_BRANCH_11_PLUS_DAY_STEM_2"; assert "hiep_ky_v30e2_yue_de_he" in s["implemented_scopes"]
    v=s["hiep_ky_v30e2"]; assert v["activated_token"]=="月徳合"; assert v["decision_effect"]=="FAVORABLE_SUPPORT_ONLY"; assert v["creates_hard_block"] is False; assert v["numeric_score"] is None
