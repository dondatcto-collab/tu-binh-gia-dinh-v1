...
def test_v2_routes_cover_first_three_user_flows():
    text = read("cong/api_v2.py")
    for route in ("/schema-status", "/hom-nay", "/thang-nay", "/tim-ngay"):
...
def test_v2_adapter_is_not_a_second_decision_engine():
    ...
    assert "score" not in text.split("def event_item", 1)[0].lower() or "numeric_score" in text
