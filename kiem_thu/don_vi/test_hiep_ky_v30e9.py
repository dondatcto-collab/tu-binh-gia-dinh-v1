...
from loi.quyet_dinh.hiep_ky_month_day_branch_v30e9 import (
    TIAN_YI_BRANCH_BY_MONTH_BRANCH,
    active_month_day_branch_tokens,
    calculator_status,
    tian_yi_branch,
)
...
def _personal(state="SUPPORT", stem=None):
    return {"state":state,"current_stem":stem,"current_branch":"TUAT","rule_ids":[],"source_ids":[],"branch_impacts":[],"theme":{},"dien_giai":{},"technical_facts":[]}


def test_tian_yi_locks_all_twelve_month_rows():
    expected={
        "DAN":"TUAT","MAO":"HOI","THIN":"TY","TI":"SUU",
        "NGO":"DAN","MUI":"MAO","THAN":"THIN","DAU":"TI",
        "TUAT":"NGO","HOI":"MUI","TY":"THAN","SUU":"DAU",
    }
    assert TIAN_YI_BRANCH_BY_MONTH_BRANCH==expected
    for month, day_branch in expected.items():
        assert tian_yi_branch(month)==day_branch
        assert active_month_day_branch_tokens(month,day_branch)==("天醫",)
...
def test_tian_yi_event_inventory_is_yi_only_for_treatment():
    ...
    for event_code in {"KHAI_TRUONG","KY_HOP_DONG","MUA_TAI_SAN","DONG_THO","NHAP_TRACH","CUOI_HOI","XUAT_HANH","DAM_PHAN","NHAM_CHUC","CAU_TAI","AN_TANG"}:
        assert all(x.token!="天醫" for x in evidence_for_event(event_code))
...
def test_tian_yi_positive_gate_for_verified_treatment_without_day_stem():
    ...
    assert "天醫" in out["active_hiep_ky_tokens"]
    assert "天醫" in out["matched_yi_tokens"]
...
def test_tian_yi_ji_conflict_month_ngo_day_dan_month_build_wins():
    out=evaluate_event_v25(_base(),_personal("SUPPORT",None),chi_thang="NGO",chi_ngay="DAN")
    assert "天醫" in out["matched_yi_tokens"]
...
def test_hard_block_still_wins_over_tian_yi():
    out=evaluate_event_v25(_base("JI"),_personal("SUPPORT",None),chi_thang="DAN",chi_ngay="TUAT")
    assert "天醫" in out["matched_yi_tokens"]
...
def test_tian_yi_does_not_leak_to_unsupported_event():
    out=evaluate_event_v25(_base(event_code="KY_HOP_DONG"),_personal("SUPPORT",None),chi_thang="DAN",chi_ngay="TUAT")
    assert "天醫" in out["active_hiep_ky_tokens"]
    assert "天醫" not in out["matched_yi_tokens"]


def test_v30e9_capability_schema_and_score_are_explicit():
    cap=capability_inventory()
    assert cap["token_count"]==81
    assert cap["active_calculable_count"]==29
    assert cap["pending_calculator_count"]==52
    assert "天醫" in cap["active_tokens"]
    assert token_capability("天醫")["calculator"]=="MONTH_BRANCH_DAY_BRANCH_V30E9"
    assert cap["extension_version"]=="V3_0E9_TIAN_YI"
    assert cap["numeric_score"] is None

    calc=calculator_status()
    assert calc["active_tokens"]==("天醫",)
    ...
    assert "hiep_ky_v30e9_tian_yi" in schema["implemented_scopes"]
    v=schema["hiep_ky_v30e9"]
    assert v["activated_token"]=="天醫"
    assert v["calculator"]=="MONTH_BRANCH_DAY_BRANCH_V30E9"
