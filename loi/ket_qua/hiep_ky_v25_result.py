"""Result schema Hiệp Kỷ mở rộng có kiểm soát; giữ tương thích V2.5–V3.0C."""
...
def _rule_block(version,status,token,vi,source,effect):
    return {"extension_version":version,"status":status,"activated_token":token,"activated_token_vi":vi,"calculator":"MONTH_BRANCH_RELATIONS_V25_V30D","source_scope":source,"coverage":COVERAGE,"decision_effect":effect,"creates_hard_block":False,"full_classical_claim":False,"numeric_score":None,"numeric_score_status":"LOCKED_OFF"}

def v25_schema_overlay(base: dict[str,Any])->dict[str,Any]:
    out=dict(base); implemented=list(out.get("implemented_scopes") or [])
    for scope in ("expanded_hiep_ky_event_search","hiep_ky_v30a_yue_xing","hiep_ky_v30b_sat_trio","hiep_ky_v30c_yue_yan","hiep_ky_v30d_shi_de"):
    ...
    out.update({"schema_version":SCHEMA_VERSION,"status":STATUS,"implemented_scopes":implemented,"pending_scopes":pending,
        ...
        "hiep_ky_v30a":_rule_block("V3_0A_YUE_XING","PARTIAL_ACTIVE_ONE_ADDITIONAL_RULE","月刑","Nguyệt Hình","欽定協紀辨方書 卷20–31 · 月表一至十二","CAUTION_ONLY"),
        "hiep_ky_v30b":{"extension_version":"V3_0B_SAT_TRIO","status":"PARTIAL_ACTIVE_THREE_ADDITIONAL_RULES","activated_tokens":["劫煞","災煞","月煞"],"activated_tokens_vi":["Kiếp Sát","Tai Sát","Nguyệt Sát"],"calculator":"MONTH_BRANCH_RELATIONS_V25_V30D","source_scope":"欽定協紀辨方書 卷20–31 · 月表一至十二","coverage":COVERAGE,"decision_effect":"CAUTION_ONLY","creates_hard_block":False,"full_classical_claim":False,"numeric_score":None,"numeric_score_status":"LOCKED_OFF"},
        "hiep_ky_v30c":_rule_block("V3_0C_YUE_YAN","PARTIAL_ACTIVE_ONE_ADDITIONAL_RULE","月厭","Nguyệt Yếm","欽定協紀辨方書 卷20–31 · 月表一至十二","CAUTION_ONLY"),
        "hiep_ky_v30d":_rule_block("V3_0D_SHI_DE","PARTIAL_ACTIVE_ONE_ADDITIONAL_RULE","時徳","Thời Đức","欽定協紀辨方書 卷五 · 總要歴/歴例","FAVORABLE_SUPPORT_ONLY"),
        "numeric_score":"LOCKED_OFF"})
    principles=list(out.get("principles") or [])
    for note in ("Hiệp Kỷ chỉ kích hoạt rule đã có bộ tính; không coi inventory cổ thư là rule đã tính được.","V3.0D mở Thời Đức (時徳) theo xuân Ngọ, hạ Thìn, thu Tý, đông Dần; đây là tín hiệu thuận hỗ trợ, không cứu HARD_BLOCK hay cảnh báo sự kiện và không cộng điểm."):
...
def event_search_v25(raw:dict[str,Any])->dict[str,Any]:
    out=event_search(raw); out["schema_version"]=SCHEMA_VERSION; out["status"]=STATUS; out["ranking_mode"]="ORDINAL_V25_HARD_BLOCK_EVENT_PERSONAL"; out["hiep_ky_coverage"]=COVERAGE; out["hiep_ky_extension"]="V3_0D_SHI_DE"; out["event_search_contract"]=EVENT_SEARCH_CONTRACT
