...
V3.0E17 mở 驛馬 theo Chi tháng + Chi ngày.
...
from loi.quyet_dinh.hiep_ky_guan_ri_v30e13 import active_guan_ri_tokens
from loi.quyet_dinh.hiep_ky_lin_ri_v30e16 import active_lin_ri_tokens
from loi.quyet_dinh.hiep_ky_min_ri_v30e15 import active_min_ri_tokens
...
COVERAGE="V3_0E17_PARTIAL_12_TRUC_PLUS_MONTH_BRANCH_11_PLUS_DAY_STEM_3_PLUS_SEASON_STEM_1_PLUS_DAY_PILLAR_1_PLUS_SEASON_DAY_PILLAR_1_PLUS_SEASON_BRANCH_1_PLUS_DAY_BRANCH_1_PLUS_MONTH_DAY_BRANCH_1_PLUS_PAIRED_MONTH_DAY_BRANCH_1_PLUS_QUARTERED_MONTH_DAY_BRANCH_1_PLUS_WANG_RI_1_PLUS_GUAN_RI_1_PLUS_XIANG_RI_1_PLUS_MIN_RI_1_PLUS_LIN_RI_1_PLUS_YI_MA_1"
ACTIVE_EXTRA_TOKENS=frozenset({"月建","月破","三合","六合","月害","月刑","劫煞","災煞","月煞","月厭","時徳","月徳","月徳合","月恩","四相","天願","天赦","天喜","五合","天醫","解神","五富","王日","官日","相日","民日","臨日","驛馬"})
...
def _effective_day_stem(personal:dict[str,Any],explicit_stem:str|None)->str|None:
    if explicit_stem in CAN: return explicit_stem
...
def evaluate_event_v25(base_event:dict[str,Any],personal:dict[str,Any],*,chi_thang:str,chi_ngay:str,can_ngay:str|None=None)->dict[str,Any]:
    ...
    active=set(active_month_tokens(chi_thang,chi_ngay)); active.update(active_season_branch_tokens(chi_thang,chi_ngay)); active.update(active_day_branch_tokens(chi_ngay)); active.update(active_month_day_branch_tokens(chi_thang,chi_ngay)); active.update(active_giai_than_tokens(chi_thang,chi_ngay)); active.update(active_wu_fu_tokens(chi_thang,chi_ngay)); active.update(active_wang_ri_tokens(chi_thang,chi_ngay)); active.update(active_guan_ri_tokens(chi_thang,chi_ngay)); active.update(active_xiang_ri_tokens(chi_thang,chi_ngay)); active.update(active_min_ri_tokens(chi_thang,chi_ngay)); active.update(active_lin_ri_tokens(chi_thang,chi_ngay)); active.update(active_yi_ma_tokens(chi_thang,chi_ngay))
    ...
    evidence=tuple(evidence_for_event(event_code)) if event_code else (); matched=[x for x in evidence if x.token in active and x.token in ACTIVE_EXTRA_TOKENS]; yi_hits=[x for x in matched if x.polarity=="YI"]; ji_hits=[x for x in matched if x.polarity=="JI"]
    base_state=out.get("event_state","NEUTRAL"); hard_block=base_state=="JI"
    if hard_block: event_signal="HARD_BLOCK"
    elif ji_hits: event_signal="CAUTION"
    elif base_state=="YI" or yi_hits: event_signal="FAVORABLE"
    else: event_signal="NEUTRAL"
    decision=resolve_conflict(hard_block=hard_block,event_state=event_signal,personal_state=_personal_signal(personal))
    if out.get("mapping_status")=="PROVISIONAL" and decision["label"]=="Ưu tiên": decision={**decision,"state":"CONSIDER","label":"Có thể cân nhắc","authority":"EVENT_PROVISIONAL"}
    reasons=list(out.get("reasons") or [])
    if yi_hits: reasons.append("Hiệp Kỷ ghi nhận thêm tín hiệu phù hợp: "+", ".join(x.token for x in yi_hits)+".")
    if ji_hits: reasons.append("Hiệp Kỷ ghi nhận tín hiệu cần thận trọng: "+", ".join(x.token for x in ji_hits)+"; lớp này chưa tự tạo HARD_BLOCK.")
    reasons.append("Phân xử theo thứ bậc HARD_BLOCK > sự kiện > cá nhân; không cộng/trừ điểm.")
    personal_context={"current_stem":personal.get("current_stem"),"current_branch":personal.get("current_branch"),"theme":personal.get("theme"),"branch_impacts":personal.get("branch_impacts",[]),"headline":personal.get("dien_giai",{}).get("headline"),"technical_facts":personal.get("technical_facts",[]),"interpretation_status":"ZPZQ_PERSONAL_0_5","decision_effect":personal.get("state","DESCRIPTIVE_ONLY")}
