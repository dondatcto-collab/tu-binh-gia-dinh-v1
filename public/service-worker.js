const CACHE = 'tubinh-v1-ui-0.5.0';
const SHELL = ['/', '/static/app.css?v=0.5.0', '/static/app.js?v=0.5.0', '/manifest.webmanifest', '/icon-192.png', '/icon-512.png'];
...
self.addEventListener('fetch', event => {
  ...
  if(req.mode==='navigate'){
    event.respondWith(fetch(req,{cache:'no-store'}).then(res=>{const copy=res.clone(); caches.open(CACHE).then(c=>c.put('/',copy)); return res;}).catch(()=>caches.match('/'))); return;
  }
