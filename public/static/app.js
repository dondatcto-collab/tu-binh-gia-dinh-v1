...
function ordinalMeter(label){const cls=badgeClass(label);return `<div class="ordinal-meter ${cls}"><div class="meter-arc"></div><strong>${esc(label||'Đang đánh giá')}</strong><small>Theo phương pháp đã khóa</small></div>`}
function descriptiveOnly(s){return s?.co_ket_luan_ca_nhan_thuan_nghich===false||s?.methodology?.decision_mode==='DESCRIPTIVE_ONLY'||s?.dien_giai?.methodology?.decision_mode==='DESCRIPTIVE_ONLY'}
async function loadHomeDashboard(){
 ...
 try{const d=await post('/api/stateless/dashboard',{profile:p});const pos=d.vi_tri||{};if(cycle)cycle.textContent=`Đại vận ${pos.dai_van?.tru||'—'} · Năm ${pos.nam_hien_tai?.vi||'—'} · Tháng ${pos.thang_hien_tai?.vi||'—'}`;
  if(monthEl){const m=d.thang||{},s=m.don_gian||{},dg=s.dien_giai||{};monthEl.classList.remove('loading-card');monthEl.innerHTML=`<button class="home-card-click" onclick="openQuestion('month')"><div class="home-card-head"><div><small>THÁNG NÀY CỦA TÔI</small><b>${esc(m.chuyen_sau?.thang?.tru?.vi||'Tháng hiện tại')}</b></div><span>›</span></div><div class="home-month-body">${ordinalMeter(s.tom_tat)}<div class="home-points"><p><i class="neutral-dot"></i><b>Xu hướng chung:</b> ${esc(s.tom_tat||'Chưa có tín hiệu nổi bật')}</p><p><i class="neutral-dot"></i><b>Tài chính:</b> ${esc(dg.tai_chinh||'Chưa đủ căn cứ')}</p><p><i class="neutral-dot"></i><b>Quan hệ:</b> ${esc(dg.quan_he||'Chưa đủ căn cứ')}</p><p><i class="neutral-dot"></i><b>Việc lớn:</b> ${esc(dg.viec_lon||'Chưa đủ căn cứ')}</p></div></div></button>`}
  if(todayEl){const t=d.hom_nay||{},s=t.don_gian||{},dg=s.dien_giai||{};todayEl.classList.remove('loading-card');todayEl.innerHTML=`<button class="home-card-click" onclick="openQuestion('today')"><div class="home-card-head"><div><small>HÔM NAY THẾ NÀO?</small><b>${esc(s.tom_tat||dg.headline||'Chưa có tín hiệu nổi bật')}</b></div><span>›</span></div><div class="today-compact"><span class="today-check neutral">i</span><div><p><b>Xu hướng:</b> ${esc(s.tom_tat||'Chưa có tín hiệu nổi bật')}</p><p><b>Quan hệ:</b> ${esc(dg.quan_he||'Chưa đủ căn cứ')}</p>${t.so_sanh_lien_ke?.ngay_mai?`<small>${esc(t.so_sanh_lien_ke.ngay_mai.so_sanh)}</small>`:''}</div></div></button>`}
...
async function loadProfileInsight(){
 ...
 try{
  ...
  <div class="card destiny-card"><div class="destiny-title"><h3>TỔNG QUAN MỆNH LÝ</h3><span>Tóm tắt</span></div><div class="destiny-row"><span>Tứ Trụ</span><b>${esc(pillars||'—')}</b></div><div class="destiny-row"><span>Đại vận hiện tại</span><b>${esc(dv?.tru||'—')}<small>${dv?`${esc(stage)} · khoảng ${esc(dv.nam_bat_dau)}–${esc(dv.nam_ket_thuc)}`:''}</small></b></div><div class="destiny-row"><span>Năm / Tháng</span><b>${esc(d.nam_hien_tai?.vi||'—')} · ${esc(d.thang_hien_tai?.vi||'—')}</b></div><div class="destiny-row"><span>Cách cục / Hỷ-Kỵ</span><b>Đã bật theo Tử Bình Chân Thuyên<small>Trường hợp chưa đủ rõ sẽ không ép kết luận</small></b></div></div>
  <div class="profile-actions-a"><button onclick="openQuestion('long')"><span>◷</span><b>Dòng thời gian</b><small>Xem vận hiện tại</small></button><button onclick="exportBackup()"><span>☁</span><b>Sao lưu hồ sơ</b><small>Lưu dữ liệu an toàn</small></button><button onclick="loadProfileWhy()"><span>▤</span><b>Giải thích</b><small>Xem cách tính & nguồn</small></button><button class="danger-lite" onclick="deleteProfile('${p.profile_id}')"><span>⌫</span><b>Xóa hồ sơ</b><small>Chỉ trên thiết bị</small></button></div>
...
function badgeClass(label=''){if(/Bị chặn|Không ưu tiên|JI|HARD_BLOCK/i.test(label))return'bad';if(/Ưu tiên|Phù hợp|Thuận nền mệnh|SUPPORT/i.test(label))return'good';if(/Có thể cân nhắc|Cân nhắc|Cần thận trọng|CAUTION|lưu ý/i.test(label))return'warn';return'neutral'}
function decisionHero(simple,scope){const label=simple.tom_tat||simple.tieu_de||'Đang đánh giá';const cls=badgeClass(label);return `<div class="decision-hero ${cls}"><div class="decision-main"><small>${scope==='month'?'THÁNG NÀY':'HÔM NAY'}</small><strong>${esc(label)}</strong><span>${esc(simple.vi_sao||'Kết luận theo các lớp V1 đã nghiệm thu, có truy nguồn.')}</span></div><div class="decision-seal">V1<br><b>0.5</b></div></div>`}
function adviceHtml(simple){if(descriptiveOnly(simple))return `<div class="notice info"><b>Chưa sinh lời khuyên cá nhân</b>Thập Thần và quan hệ Can/Chi hiện chỉ mô tả cấu trúc. App sẽ chỉ hiện Nên / Hạn chế sau khi Cách cục và hỷ-kỵ mệnh gốc được xác lập.</div>`;return `<div class="advice-grid"><div class="advice good"><h4>✓ NÊN</h4>${(simple.nen_lam||[]).map(x=>`<p>• ${esc(x)}</p>`).join('')}</div><div class="advice warn"><h4>! CÂN NHẮC</h4>${(simple.can_nhac||[]).map(x=>`<p>• ${esc(x)}</p>`).join('')}</div><div class="advice bad"><h4>× KHÔNG ƯU TIÊN</h4>${(simple.khong_uu_tien||[]).map(x=>`<p>• ${esc(x)}</p>`).join('')}</div></div>`}
function evidenceFactors(s){const d=s?.dien_giai||{},arr=d.yeu_to_chinh||[];if(!arr.length)return'';return `<div class="evidence-factors"><h3>3 yếu tố tạo ra kết luận</h3>${arr.map((x,i)=>`<div class="evidence-row"><span>${i+1}</span><div><b>${esc(x.title||'Yếu tố')}</b><p>${esc(x.plain||'')}</p></div></div>`).join('')}</div>`}
function concreteActions(s){const d=s?.dien_giai||{},yes=d.nen_cu_the||[],no=d.tranh_cu_the||[];if(!yes.length&&!no.length)return'';return `<div class="concrete-actions"><div><h3>NÊN LÀM CỤ THỂ</h3>${yes.map(x=>`<p>✓ ${esc(x)}</p>`).join('')}</div><div><h3>TRÁNH / HẠN CHẾ</h3>${no.map(x=>`<p>• ${esc(x)}</p>`).join('')}</div></div>`}
function adjacentCompare(d,scope){const c=d?.so_sanh_lien_ke||{};const keys=scope==='month'?[['thang_truoc','Tháng trước'],['thang_sau','Tháng sau']]:[['hom_qua','Hôm qua'],['ngay_mai','Ngày mai']];const rows=keys.map(([k,l])=>c[k]?`<div class="compare-row"><b>${l}</b><p>${esc(c[k].so_sanh||'')}</p></div>`:'').join('');return rows?`<div class="adjacent-compare"><h3>ĐIỂM KHÁC BIỆT LIỀN KỀ</h3>${rows}</div>`:''}
...
function hoursBlock(d){const arr=(d.gio_trong_ngay||[]).filter(x=>x.relation&&x.relation!=='NONE');return `<div class="hours-card"><h4>QUAN HỆ CHI GIỜ VỚI HỒ SƠ</h4><p class="muted">Chỉ hiển thị quan hệ cấu trúc, chưa gọi giờ tốt/xấu.</p><div class="hour-grid">${arr.slice(0,6).map(x=>`<span><b>${esc(x.khoang_gio)}</b><small>${esc(x.chi_vi)} · ${esc(x.nhan)}</small></span>`).join('')||'<span class="muted">Chưa có quan hệ trực tiếp nổi bật</span>'}</div></div>`}
function renderTodayA(d,p){const s=d.don_gian,deep=d.chuyen_sau;return `<div class="today-a-hero ${badgeClass(s.tom_tat)}"><div><small>TỔNG QUAN HÔM NAY</small><h2>${esc((s.dien_giai||{}).headline||s.tom_tat)}</h2><p>${esc(s.tom_tat)}</p></div><div class="ordinal-disc"><span>V1</span><b>0.5</b></div></div>${evidenceFactors(s)}${impactGrid(s)}${concreteActions(s)}${adjacentCompare(d,'day')}${hoursBlock(d)}<div class="card why-summary"><h3>TẠI SAO HÔM NAY NHƯ VẬY?</h3>${fusionHtml(deep,true)}<p class="muted">Chuỗi Đại vận → Năm → Tháng → Ngày được hợp lưu theo cấu trúc. Trường hợp Cách cục/Hỷ-Kỵ chưa đủ rõ sẽ tự hạ về mô tả, không ép kết luận.</p><button class="btn full-btn" onclick="loadWhy('day')">Xem chi tiết</button><div id="why-box"></div></div>`}
function renderMonthB(d,p){const s=d.don_gian,deep=d.chuyen_sau;const dg=s.dien_giai||{};return `<div class="month-b-timeline">${fusionHtml(deep,false)}</div><div class="month-b-summary"><div class="month-compass">◈</div><div><small>ĐÁNH GIÁ THÁNG</small><h2>${esc(dg.headline||s.tom_tat)}</h2><p>${esc(s.tom_tat)}</p></div></div>${evidenceFactors(s)}${impactGrid(s)}${concreteActions(s)}${adjacentCompare(d,'month')}<button class="btn secondary full-btn" onclick="loadWhy('month')">TẠI SAO? · Xem chuyên sâu</button><div id="why-box"></div>`}
...
function calendarTone(label,state=''){const x=(label||'')+' '+(state||'');if(/Không ưu tiên|JI|CAN_NHAC|xung động|Cần thận trọng|Cân nhắc|CAUTION/i.test(x))return /Không ưu tiên|JI/i.test(x)?'bad':'warn';if(/Ưu tiên|Phù hợp|THUAN|Khá thuận|Thuận nền mệnh|SUPPORT/i.test(x))return 'good';return 'neutral'}
...
async function findDates(){
 ...
 try{
  ...
  const first=rows[0];$('work-result').innerHTML=`${d.canh_bao_an_toan?`<div class="notice danger"><b>Lưu ý y tế</b>${esc(d.canh_bao_an_toan)}</div>`:''}${first?`<div class="work-winner"><small>XẾP HẠNG #1 · 12 TRỰC V1 + CÁ NHÂN</small><div class="winner-row"><div class="medal">1</div><div class="grow"><h2>${esc(first.ngay)}</h2><strong>${esc(first.label)}</strong><p>${first.hours.length?'Quan hệ Chi giờ với hồ sơ: '+first.hours.map(h=>`${esc(h.chi_vi)} (${esc(h.khoang_gio)})`).join(' · '):'Giờ hiện chỉ là tham khảo cấu trúc; chưa kết luận tốt/xấu cá nhân.'}</p></div><span class="ordinal-badge">V1<br>0.5</span></div></div>`:''}<div class="card work-other"><h3>DANH SÁCH GỢI Ý KHÁC</h3>${rows.slice(1).map((x,i)=>`<div class="work-rank-row"><div class="rank-no">${i+2}</div><div class="grow"><b>${esc(x.ngay)}</b><strong>${esc(x.label)}</strong><small>Trực ${esc(x.truc||'—')} · Nền cá nhân: ${esc(x.personal_v1_1?.headline||'chưa có tín hiệu nổi bật')}</small>${x.hours.length?`<p>Quan hệ Chi giờ với hồ sơ: ${x.hours.map(h=>esc(h.chi_vi)).join(', ')}</p>`:''}</div><span>›</span></div>`).join('')||'<div class="muted">Không có gợi ý khác.</div>'}<div class="soft-note">${esc(d.ghi_chu||'')}</div><div class="work-buttons"><button class="btn secondary" onclick="document.querySelector('.work-search-card')?.scrollIntoView({behavior:'smooth'})">Chỉnh sửa tìm kiếm</button><button class="btn" onclick="toggleResearchDays()">Xem chi tiết tất cả</button></div><div id="research-days" class="research-days" hidden>${(d.cac_ngay||[]).map(x=>`<div class="research-row"><b>${x.ngay}</b> · ${esc(x.tru_ngay)} · <span class="result-label ${badgeClass(x.label)}">${esc(x.label)}</span><div class="muted">Trực ${esc(x.truc||'—')} · Nền cá nhân: ${esc(x.personal_v1_1?.headline||'chưa có tín hiệu nổi bật')}</div></div>`).join('')}</div></div>`;
