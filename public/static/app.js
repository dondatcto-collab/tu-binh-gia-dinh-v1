...
async function loadHomeDashboard(){
 ...
 try{const d=await post('/api/stateless/dashboard',{profile:p});const pos=d.vi_tri||{};if(cycle)cycle.textContent=`Đại vận ${pos.dai_van?.tru||'—'} · ${pos.nam_hien_tai?.vi||'—'} · ${pos.thang_hien_tai?.vi||'—'}`;
  ...
  if(todayEl){const t=d.hom_nay||{},s=t.don_gian||{};todayEl.classList.remove('loading-card');const good=(t.gio_trong_ngay||[]).filter(x=>x.relation_level==='POSITIVE').slice(0,3);todayEl.innerHTML=`<button class="home-card-click" onclick="openQuestion('today')"><div class="home-card-head"><div><small>HÔM NAY THẾ NÀO?</small><b class="${badgeClass(s.tom_tat)}-text">${esc(s.tom_tat)}</b></div><span>›</span></div><div class="today-compact"><span class="today-check">✓</span><div><p>${esc(s.vi_sao||'Kết luận theo lớp V1-basic có truy nguồn.')}</p>${good.length?`<small>Giờ tham khảo theo hồ sơ: ${good.map(x=>esc(x.chi_vi)).join(', ')}</small>`:''}</div></div></button>`}
...
function renderTodayA(d,p){const s=d.don_gian,deep=d.chuyen_sau;return `<div class="today-a-hero ${badgeClass(s.tom_tat)}"><div><small>TỔNG QUAN HÔM NAY</small><h2>${esc(s.tom_tat)}</h2><p>${esc(s.vi_sao||'Kết luận theo lớp V1-basic có truy nguồn.')}</p></div><div class="ordinal-disc"><span>V1</span><b>BASIC</b></div></div>${adviceHtml(s)}${hoursBlock(d)}<div class="card why-summary"><h3>TẠI SAO HÔM NAY NHƯ VẬY?</h3>${fusionHtml(deep,true)}<p class="muted">Kết luận hợp lưu Đại vận → Năm → Tháng → Ngày. Bấm dưới đây để xem cách tính và nguồn.</p><button class="btn full-btn" onclick="loadWhy('day')">Xem chi tiết</button><div id="why-box"></div></div>`}
