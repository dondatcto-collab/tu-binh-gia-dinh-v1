...
async function loadProfileInsight(){
 ...
 try{
  ...
  <div class="card destiny-card"><div class="destiny-title"><h3>TỔNG QUAN MỆNH LÝ</h3><span>Tóm tắt</span></div><div class="destiny-row"><span>Tứ Trụ</span><b>${esc(pillars||'—')}</b></div><div class="destiny-row"><span>Đại vận hiện tại</span><b>${esc(dv?.tru||'—')}<small>${dv?`${esc(stage)} · khoảng ${esc(dv.nam_bat_dau)}–${esc(dv.nam_ket_thuc)}`:''}</small></b></div><div class="destiny-row"><span>Năm / Tháng</span><b>${esc(d.nam_hien_tai?.vi||'—')} · ${esc(d.thang_hien_tai?.vi||'—')}</b></div><div class="destiny-row"><span>Cách cục / Hỷ-Kỵ</span><b class="muted">Đang hoàn thiện theo Tử Bình Chân Thuyên</b></div></div>
...
function renderTodayA(d,p){const s=d.don_gian,deep=d.chuyen_sau;return `<div class="today-a-hero ${badgeClass(s.tom_tat)}"><div><small>TỔNG QUAN HÔM NAY</small><h2>${esc((s.dien_giai||{}).headline||s.tom_tat)}</h2><p>${esc(s.tom_tat)}</p></div><div class="ordinal-disc"><span>V1</span><b>BASIC</b></div></div>${evidenceFactors(s)}${impactGrid(s)}${concreteActions(s)}${adjacentCompare(d,'day')}${hoursBlock(d)}<div class="card why-summary"><h3>TẠI SAO HÔM NAY NHƯ VẬY?</h3>${fusionHtml(deep,true)}<p class="muted">Chuỗi Đại vận → Năm → Tháng → Ngày đang được đọc theo cấu trúc. Chưa phán thuận/nghịch cá nhân khi nền Cách cục/Hỷ-Kỵ chưa đủ.</p><button class="btn full-btn" onclick="loadWhy('day')">Xem chi tiết</button><div id="why-box"></div></div>`}
...
async function findDates(){
 ...
 try{
  ...
  const first=rows[0];$('work-result').innerHTML=`${d.canh_bao_an_toan?`<div class="notice danger"><b>Lưu ý y tế</b>${esc(d.canh_bao_an_toan)}</div>`:''}${first?`<div class="work-winner"><small>NGÀY XẾP HẠNG #1 THEO HIỆP KỶ</small><div class="winner-row"><div class="medal">1</div><div class="grow"><h2>${esc(first.ngay)}</h2><strong>${esc(first.label)}</strong><p>${first.hours.length?'Quan hệ Chi giờ với hồ sơ: '+first.hours.map(h=>`${esc(h.chi_vi)} (${esc(h.khoang_gio)})`).join(' · '):'Giờ tốt/xấu cá nhân chưa được kết luận ở 0.4.0.'}</p></div><span class="ordinal-badge">V1<br>BASIC</span></div></div>`:''}<div class="card work-other"><h3>DANH SÁCH GỢI Ý KHÁC</h3>${rows.slice(1).map((x,i)=>`<div class="work-rank-row"><div class="rank-no">${i+2}</div><div class="grow"><b>${esc(x.ngay)}</b><strong>${esc(x.label)}</strong><small>Trực ${esc(x.truc||'—')} · Quan hệ cấu trúc: ${esc(x.personal_relation?.nhan||'chưa có')}</small>${x.hours.length?`<p>Quan hệ Chi giờ với hồ sơ: ${x.hours.map(h=>esc(h.chi_vi)).join(', ')}</p>`:''}</div><span>›</span></div>`).join('')||'<div class="muted">Không có gợi ý khác.</div>'}<div class="soft-note">${esc(d.ghi_chu||'')}</div><div class="work-buttons"><button class="btn secondary" onclick="document.querySelector('.work-search-card')?.scrollIntoView({behavior:'smooth'})">Chỉnh sửa tìm kiếm</button><button class="btn" onclick="toggleResearchDays()">Xem chi tiết tất cả</button></div><div id="research-days" class="research-days" hidden>${(d.cac_ngay||[]).map(x=>`<div class="research-row"><b>${x.ngay}</b> · ${esc(x.tru_ngay)} · <span class="result-label ${badgeClass(x.label)}">${esc(x.label)}</span><div class="muted">Trực ${esc(x.truc||'—')} · Quan hệ cấu trúc: ${esc(x.personal_relation?.nhan||'chưa có')}</div></div>`).join('')}</div></div>`;
